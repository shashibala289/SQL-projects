...
--1.Query 1: Retrieve the number of customers per month.
...
--2.Query 2: Retrieve the Total Hotel sales per month.
...
--3.Query 3: Retrieve Average Satisfaction rating per month.
select avg(sf.satisfaction_level), to_char(t.payment_date,'MM')MM from transactions t,
satisfaction sf where sf.trans_id=t.transaction_id  
group by to_char(payment_date, 'MM')
order by to_char(payment_date, 'MM')asc;
